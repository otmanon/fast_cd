...
class QuadraticRotatedMatrix
{
public:
	//class that aids in the computation of D = U * R * H * R * V, where R is a 3nx3n block diagonal matrix of per vertex transformations. 
	...
	QuadraticRotatedMatrix(const Eigen::MatrixXd& U, const Eigen::MatrixXd& V, const Eigen::SparseMatrix<double>& H, const Eigen::MatrixXd& W);

	QuadraticRotatedMatrix() {};
...
public:
	...
	std::vector<std::vector<std::vector<std::vector<std::vector<Eigen::MatrixXd>>>>> UEHEV; // list of 9 matrices per each bone such that UEV_uvb p_uvb = U * T * V
};