...
{
public:
    //keep this around, even though we wonb't use it anymore
    PhysicsHook() : sim_thread(NULL), please_pause(false), please_die(false), running(false)
    {
    }

    PhysicsHook(igl::opengl::glfw::Viewer* viewer) : sim_thread(NULL), please_pause(false), please_die(false), running(false)
    {
   ...
   // virtual void init_viewer(igl::opengl::glfw::Viewer* viewer) = 0;
    /*
    * Called every once in a while (and always before every simulation reduced_step) even if the simulation is paused.
    * Use this to update the visualization in response to user input etc.
    */
    virtual void tick() {};

    /*
     * Takes one simulation "reduced_step." You can do whatever you want here, but the granularity of your computation should 
     * be small enough that the user can view/pause/kill the simulation at interactive rates.
     * This method *must* be thread-safe with respect to render() (easiest is to not touch any rendering
    ...
    */
    ...
    /*
     * Pause a running simulation. The simulation will pause at the end of its current "reduced_step"; this method will not
    ...
    void runSimThread()
    {
     ...
     //   status_mutex.unlock();
        ...
        while (!done)
        {
            ...
            {
                    //only call on this thread to match 60 fps
